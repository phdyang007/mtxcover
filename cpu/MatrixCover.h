...
c2|   1              1
 */
//typename MCSolverTraitsType::MatrixType **dl_matrix;
...
//int selected_row_id
//typename MCSolverTraitsType::ResultType *results;
...
//
template <typename T>
void print_vec(T *vec, int vec_length) {
...
template <typename T>
void print_matrix(T **mat, int row, int col) {
  for (int i = 0; i < row; i++) {
    for (int j = 0; j < col; j++) {
      std::cout << mat[i][j] << ' ';
    }
...
void delete_rows_and_columns(typename MCSolverTraitsType::MatrixType **dl_matrix, int *deleted_rows,
...
template <typename T>
void init_vectors(T *vec, int vec_length, T value = 0) {
  for (int i = 0; i < vec_length; i++) {
    vec[i] = value;
  }
}
...
//
template <typename T>
int get_largest_value(T *vec, int vec_length) {
...
//
template <typename MCSolverTraitsType>
bool check_existance_of_candidate_rows(int *deleted_rows, typename MCSolverTraitsType::RowGroupType *row_group,
                                       int search_depth,
...
template <typename MCSolverTraitsType>
void get_vertex_row_group(typename MCSolverTraitsType::RowGroupType *row_group, typename MCSolverTraitsType::MatrixType **dl_matrix, int vertex_num,
                          int total_dl_matrix_row_num) {
...
//select one row
template <typename MCSolverTraitsType>
int select_row(int *deleted_rows, typename MCSolverTraitsType::RowGroupType *row_group, int search_depth,
               int total_dl_matrix_row_num) {
  for (int i = 0; i < total_dl_matrix_row_num; i++) {
    if (deleted_rows[i] == 0 && row_group[i] == search_depth) {
      return i;
...
//
template <typename MCSolverTraitsType>
void recover_results(typename MCSolverTraitsType::ResultType *results, int search_depth,
                     int total_dl_matrix_row_num) {
  for (int i = 0; i < total_dl_matrix_row_num; i++) {
...
//
template <typename MCSolverTraitsType>
int get_conflict_node_id(int *deleted_rows, typename MCSolverTraitsType::RowGroupType *row_group, int search_depth,
                         int total_dl_matrix_row_num) {
...
//
template <typename MCSolverTraitsType>
int get_conflict_col(typename MCSolverTraitsType::MatrixType **dl_matrix, int *deleted_rows, int *deleted_cols,
                     typename MCSolverTraitsType::RowGroupType *row_group, int conflict_node_id, int search_depth,
...
template <typename MCSolverTraitsType>
void remove_cols(int *deleted_cols, typename MCSolverTraitsType::ColGroupType *col_group, int conflict_col_id,
                 int total_dl_matrix_col_num) {
  for (int i = 0; i < total_dl_matrix_col_num; i++) {
...
//
template <typename MCSolverTraitsType>
void mc_solver(typename MCSolverTraitsType::MatrixType **dl_matrix, typename MCSolverTraitsType::ResultType *results, int *deleted_cols, typename MCSolverTraitsType::ColGroupType *col_group,
               int vertex_num, int total_dl_matrix_row_num,
  ...
  typename MCSolverTraitsType::ConflictCountType *conflict_count = new typename MCSolverTraitsType::ConflictCountType [total_dl_matrix_col_num];

  int *deleted_rows = new int[total_dl_matrix_row_num];

  int *vertices_covered = new int[vertex_num];
  typename MCSolverTraitsType::RowGroupType *row_group = new typename MCSolverTraitsType::RowGroupType [total_dl_matrix_row_num];
  // typename MCSolverTraitsType::ColGroupType *col_group = new typename MCSolverTraitsType::ColGroupType [total_dl_matrix_col_num];
  ...
  get_vertex_row_group<MCSolverTraitsType>(row_group, dl_matrix, vertex_num,
  ...
  for (search_depth = 1; search_depth <= vertex_num;) {
    token=check_existance_of_candidate_rows<MCSolverTraitsType>(
    ...
    if (token) { // check if there are candidate rows
      ...
      selected_row_id =
          select_row<MCSolverTraitsType>(deleted_rows, row_group, search_depth,
      ...
      delete_rows_and_columns<MCSolverTraitsType>(
...
#endif      
      if (search_depth > 0) {
        conflict_node_id = get_conflict_node_id<MCSolverTraitsType>(
            deleted_rows, row_group, search_depth, total_dl_matrix_row_num);
        if(conflict_node_id>0){
          conflict_col_id = get_conflict_col<MCSolverTraitsType>(
          ...
          recover_deleted_cols(deleted_cols, search_depth,
                              total_dl_matrix_col_num); // recover deleted cols
                                                        ...
                                                        // previously selected
                                                        // rows
          recover_results<MCSolverTraitsType>(results, search_depth,
          ...
          if (conflict_count[conflict_col_id] > hard_conflict_threshold) {
            ...
            init_vectors(deleted_rows, total_dl_matrix_row_num);
            init_vectors(results, total_dl_matrix_row_num);
            remove_cols<MCSolverTraitsType>(deleted_cols, col_group, conflict_col_id,
