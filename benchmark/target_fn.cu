...
// vertex_num
// };
MeasureTimer Invoke_ORIGINAL_CPU(DataSet *dataset, bool print_result, int &backtrace_num, int &total_conflict_num) {
  MeasureTimer timer;
 ...
 // int backtrace_num = 0;
  ...
  mc_solver(dl_matrix, results.data(), deleted_cols.data(),
            ...
            total_col, backtrace_num, dataset->search_order.data());
  timer.EndCoreTime();
...
//  total_conflict_num = total_conflict_num + conflict_count/3;
  ...
  if (print_result) {
    ...
    //std::cout << "Backtrace Num is " << backtrace_num << std::endl;
    total_conflict_num = total_conflict_num + conflict_count;
...
MeasureTimer Invoke_ORIGINAL_GPU(DataSet *dataset, bool print_result, int &backtrace_num, int &total_conflict_num) {
  MeasureTimer timer;
...
MeasureTimer Invoke_ORIGINAL_GPU_MG(DataSets *datasets, bool print_result, int &backtrace_num, int &total_conflict_num) {
  MeasureTimer timer;
...
MeasureTimer Invoke(const ImplVersion version, bool print_result,
  ...
  case ImplVersion::ORIGINAL_CPU:
    return Invoke_ORIGINAL_CPU(dataset, print_result, backtrace_num, total_conflict_num);
  case ImplVersion::ORIGINAL_GPU:
    return Invoke_ORIGINAL_GPU(dataset, print_result, backtrace_num, total_conflict_num);
  default:
...
MeasureTimer Invoke(const ImplVersion version, bool print_result,
  ...
  case ImplVersion::ORIGINAL_GPU_MG:
    return Invoke_ORIGINAL_GPU_MG(datasets, print_result , backtrace_num, total_conflict_num);
  default:
    std::cout << "Not Impl yet" << std::endl;
    return default_timer;
  }
