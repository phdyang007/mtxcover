...
int main() {
  ...
  cudaMemcpy(dl_matrix_gpu, dl_matrix,
             sizeof(int) * total_dl_matrix_row_num * total_dl_matrix_col_num,
  ...
  gpu::init_vectors<<<1, 32>>>(row_group_gpu, total_dl_matrix_row_num_gpu);
  ...
  cudaMemcpy(row_group, row_group_gpu,
  ...
  std::cout << std::endl;
  cudaMemcpy(col_group_gpu, col_group, sizeof(int) * total_dl_matrix_col_num,
             cudaMemcpyHostToDevice);

  // delete_rows_and_columns<<<1, 32>>>(dl_matrix_gpu, deleted_rows_gpu,
  // deleted_cols_gpu, 1, 1, total_dl_matrix_row_num, total_dl_matrix_col_num);

  // cudaMemcpy(deleted_cols, deleted_cols_gpu,
  // sizeof(int)*total_dl_matrix_col_num, cudaMemcpyDeviceToHost);
  // cudaMemcpy(deleted_rows, deleted_rows_gpu,
  // sizeof(int)*total_dl_matrix_row_num, cudaMemcpyDeviceToHost);
  ...
  int *results = new int[total_dl_matrix_row_num];
  cudaMemcpy(results, results_gpu, sizeof(int) * total_dl_matrix_row_num,
